...
import (
	...
	constant2 "github.com/xiaohuashifu/him/service/user/profile/constant"
	"github.com/xiaohuashifu/him/service/user/profile/db"
	"gorm.io/gorm"
	"log"
	"time"
)
...
func (t *UserAvatarClearTask) start() {
	c := cron.New()
	_, _ = c.AddFunc(constant2.UserAvatarClearTaskCron, t.clear)
	go c.Start()
}
...
func (t *UserAvatarClearTask) getBloom() *bloom.BloomFilter {
	filter := bloom.NewWithEstimates(constant2.UserAvatarClearTaskBloomLength, constant2.UserAvatarClearTaskBloomFP)
	var avatars []string
	if err := t.db.Model(&db.UserProfile{}).Select("avatar").Find(&avatars).Error; err != nil {
		log.Fatal(err)
	}
	avatarHostLength := len(constant2.UserAvatarBucketURL)
...
func (t *UserAvatarClearTask) clearUnLinkAvatar(avatars []cos.Object, filter *bloom.BloomFilter) {
	// 头像修改时间在这个时间之前就被视为过期
	expireTime := time.Now().Add(-constant2.UserAvatarClearTaskAvatarExpireTime).Unix()
