...
type Group struct {
	...
	IsJoinApplication            bool   `gorm:"not null; type:tinyint(1) unsigned"` // 是否接受入群申请（默认不需要申请直接入群）
	IsInviteJoinGroupNeedConfirm bool   `gorm:"not null; type:tinyint(1) unsigned"` // 是否邀请入群需要管理员或群主确认（默认不需要确认直接入群）
...
type GroupMember struct {
	...
	Role           string `gorm:"not null; size:10; index"`                    // 角色
